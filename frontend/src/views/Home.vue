...
export default {
  ...
  methods: {
    myFunction () {
      ...
      // TODO: Second value is not correct nor exact, needs to be later modified so that it includes the carousel height
      if (window.pageYOffset > sticky) { filter.classList.add("sticky"); }
      if (window.pageYOffset < 400) { filter.classList.remove("sticky"); }
    }
  }
};
</script>
